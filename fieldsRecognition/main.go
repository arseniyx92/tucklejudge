...
func diminishWhiteFiguresThickness(init *image.Gray) *image.Gray {
	img := image.NewGray(init.Bounds())
	dirX := []int{-1, 0, 0, 1}
	dirY := []int{0, -1, 1, 0}
		...
		for y := init.Bounds().Min.Y; y < init.Bounds().Max.Y; y++ {
			...
			if init.GrayAt(x, y).Y == 255 {
				...
				for dir := 0; dir < 4; dir++ {
					nx := x + dirX[dir]
					ny := y + dirY[dir]
					if nx >= 0 && nx < init.Bounds().Size().X && ny >= 0 && ny < init.Bounds().Size().Y && init.GrayAt(nx, ny).Y == 255 {
						cnt++
					}
				}
				if cnt < 4 {
...
func centralizeMainComponentForAnalytics(img *image.Gray, extend bool) {
	components := getWhiteComponents(img)
	if len(components) > 1 {
	...
	centerX /= len(comp)
	centerY /= len(comp)
	scale := max(float64(boundsY.second-boundsY.first)/23., float64(boundsX.second-boundsX.first)/23.)
	...
	for i := -28; i < 28; i++ {
		for j := -28; j < 28; j++ {
			shiftX := 13
			shiftY := 14
			img.SetGray(i+shiftX+(boundsX.second-boundsX.first)/2-boundsX.second, j+shiftY+(boundsY.second-boundsY.first)/2-boundsY.second, cpy.GrayAt(int(math.Round(scale*float64(i+centerX))), int(math.Round(scale*float64(j+centerY)))))
		}
	}
	img.Rect = image.Rect(0, 0, 28, 28)
}

func centralizeMainComponent(img *image.Gray) {
	components := getWhiteComponents(img)
	if len(components) > 1 {
		for i, comp := range components {
			if i != 0 {
	...
	cpy := copyGray(img)
	boundsX := IntPair{img.Bounds().Max.X, 0}
	boundsY := IntPair{img.Bounds().Max.Y, 0}
	comp := components[0]
	for _, pos := range comp {
		boundsX.first = min(boundsX.first, pos.first)
		boundsX.second = max(boundsX.second, pos.first)
	...
	scale := max(float64(boundsY.second-boundsY.first)/27., float64(boundsX.second-boundsX.first)/30.)
	for i := -img.Bounds().Max.X; i < img.Bounds().Max.X; i++ {
		for j := -img.Bounds().Max.Y; j < img.Bounds().Max.Y; j++ {
			x := int(float64(i)*scale + centerX)
			y := int(float64(j)*scale + centerY)
			nx := i + 13
			ny := j + 26 + int((centerY-float64(boundsY.second))/scale)
			if nx >= 0 && nx < 28 && ny >= 0 && ny < 28 && x >= 0 && x < img.Bounds().Max.X && y >= 0 && y < img.Bounds().Max.Y {
				img.SetGray(nx, ny, cpy.GrayAt(x, y))
			} else {
				img.SetGray(nx, ny, color.Gray{Y: 0})
			}
		}
	}
	img.Rect = image.Rect(0, 0, 28, 28)
...
func imageFragmentTo28x28AnalyticsVersion(rec image.Rectangle, init *image.Gray, extended bool) (dims, result []int) {
	img := image.NewGray(image.Rect(0, 0, max(rec.Size().X, 28), max(rec.Size().Y, 28)))
	for x := rec.Min.X; x <= rec.Max.X; x++ {
		for y := rec.Min.Y; y <= rec.Max.Y; y++ {
			img.SetGray(x-rec.Min.X, y-rec.Min.Y, init.GrayAt(x, y))
	...
	// applying inevitable alterations
	easyThreshold(img)
	saveOnlyMainComponent(img)
	otsuThreshold(img)
	...
	{
		ok := false
		for i := img.Rect.Max.X / 3; i <= img.Rect.Max.X-img.Rect.Max.X/3; i++ {
			for j := img.Rect.Max.Y / 3; j <= img.Rect.Max.Y-img.Rect.Max.Y/3; j++ {
	...
	// diminishing picture size to fired pixels zone
	{
		bounds := []int{img.Bounds().Max.X, img.Bounds().Max.Y, 0, 0}
		for i := 0; i < img.Bounds().Max.X; i++ {
			for j := 0; j < img.Bounds().Max.Y; j++ {
				if img.GrayAt(i, j).Y > 0 {
					bounds[0] = min(bounds[0], i)
					bounds[1] = min(bounds[1], j)
					bounds[2] = max(bounds[2], i+1)
					bounds[3] = max(bounds[3], j+1)
				}
			}
		}
		eps := 1
		rec = image.Rect(bounds[0]-eps, bounds[1]-eps, bounds[2]+eps, bounds[3]+eps)
		nimg := image.NewGray(image.Rect(0, 0, rec.Size().X, rec.Size().Y))
		for i := rec.Min.X; i < rec.Max.X; i++ {
			for j := rec.Min.Y; j < rec.Max.Y; j++ {
				nimg.SetGray(i-rec.Min.X, j-rec.Min.Y, img.GrayAt(i, j))
			}
		}
		img = copyGray(nimg)
	}

	result = make([]int, img.Bounds().Max.X*img.Bounds().Max.Y)
	for x := 0; x < img.Bounds().Max.X; x++ {
		for y := 0; y < img.Bounds().Max.Y; y++ {
			result[x*img.Bounds().Max.Y+y] = int(img.GrayAt(x, y).Y)
		}
	}
	return []int{img.Bounds().Max.X, img.Bounds().Max.Y}, result
}

func imageFragmentTo28x28cnnVersion(rec image.Rectangle, init *image.Gray) (result []int) {
	img := image.NewGray(image.Rect(0, 0, max(rec.Size().X, 28), max(rec.Size().Y, 28)))
	for x := rec.Min.X; x <= rec.Max.X; x++ {
		for y := rec.Min.Y; y <= rec.Max.Y; y++ {
			img.SetGray(x-rec.Min.X, y-rec.Min.Y, init.GrayAt(x, y))
	...
	// diminishing picture size to fired pixels zone
	{
		...
		dist := max(28, max(bounds[2]-bounds[0], bounds[3]-bounds[1]))
		eps := 3
		rec = image.Rect(bounds[0]-eps, bounds[1]-eps, bounds[0]+dist+eps, bounds[1]+dist+eps)
		nimg := image.NewGray(image.Rect(0, 0, rec.Size().X, rec.Size().Y))
		for i := rec.Min.X; i <= rec.Max.X; i++ {
			for j := rec.Min.Y; j <= rec.Max.Y; j++ {
				nimg.SetGray(i-rec.Min.X, j-rec.Min.Y, img.GrayAt(i, j))
	...
	for x := 0; x < 28; x++ {
		for y := 0; y < 28; y++ {
			result[y*28+x] = int(img.GrayAt(x, y).Y)
		}
	}
	return result
}

func imageFragmentTo28x28PerceptronVersion(rec image.Rectangle, init *image.Gray) (result []int) {
	img := image.NewGray(image.Rect(0, 0, max(rec.Size().X, 28), max(rec.Size().Y, 28)))
	for x := rec.Min.X; x <= rec.Max.X; x++ {
		for y := rec.Min.Y; y <= rec.Max.Y; y++ {
			img.SetGray(x-rec.Min.X, y-rec.Min.Y, init.GrayAt(x, y))
	...
	//augmenting white pixels count
	{
		...
		//	img = diminishWhiteFiguresThickness(img)
		//}
		for getPureWhites() < 100 {
			img = augmentWhiteFiguresThickness(img)
		}
...
func gaussianBlur(img *image.Gray, kernel int) {
	...
	for i := lx; i < rx; i++ {
		for j := ly; j < ry; j++ {
			maxX := min(rx-1, i+kernel-1)
			maxY := min(ry-1, j+kernel-1)
...
func gaussianBlurNoSugar(img *image.Gray, kernel int) {
	lx := img.Bounds().Min.X
	ly := img.Bounds().Min.Y
	rx := img.Bounds().Max.X
	ry := img.Bounds().Max.Y
	...
	for i := lx; i <= rx; i++ {
		for j := ly; j <= ry; j++ {
			...
			img.SetGray(i, j, color.Gray{Y: max(img.GrayAt(i, j).Y, uint8(float64(sum)/float64(cnt)))})
		}
	}
}
...
func easyThreshold(img *image.Gray) {
	lx := img.Bounds().Min.X
	ly := img.Bounds().Min.Y
	rx := img.Bounds().Max.X
	ry := img.Bounds().Max.Y
	...
	mean = int(math.Round(float64(mean) / float64(sz)))
	for i := lx; i < rx; i++ {
		for j := ly; j < ry; j++ {
			if img.GrayAt(i, j).Y < uint8(mean) {
				img.SetGray(i, j, color.Gray{Y: 0})
			}
		}
	}
...
func formValuesProcessing(init image.Image) (results []string) {
	if !PERCEPTRON {
		...
		//NN.LoadFullyConnectedLayer(10). // 0-9
		//	AddSoftmaxLayer()

		//NN.LoadFullyConnectedLayer(10).
		//	AddReLULayer().
		//	LoadFullyConnectedLayer(10). // 0-9
		//	AddSoftmaxLayer()

		NN.LoadConvolutionLayer([]int{3, 3}, 10).
			AddMaxPoolingLayer(5, []int{4, 4}).
			LoadFullyConnectedLayer(10). // 0-9
			AddSoftmaxLayer()

		//NN.LoadConvolutionLayer([]int{3, 3}, 10).
		//	AddMaxPoolingLayer(5, []int{4, 4}).
		//	LoadFullyConnectedLayer(10).
		//	AddReLULayer().
		//	LoadFullyConnectedLayer(10). // 0-9
		//	AddSoftmaxLayer()
	}
	...
	for fieldID, field := range fields {
		...
		for _, v := range field {
			minX = min(minX, v.first)
			maxX = max(maxX, v.first+1)
			minY = min(minY, v.second)
			maxY = max(maxY, v.second+1)
		}
		blocks := 8
		borders := int(float64(maxY-minY) / 6.)
		if fieldID < 2 {
			blocks = 4
			borders = int(float64(maxY-minY) / 15.)
		}
		...
		for block := 0; block < blocks; block++ {
			...
			digit := 10
			blockRect := image.Rect(start+borders, minY+borders, finish-1-borders/2, maxY-borders)

			// digits is an array of all possible digits dedicated to a current image
			digits := AI.GetAnalyticsPrediction(imageFragmentTo28x28AnalyticsVersion(blockRect, img, true))

			// getting predictions
			predictions := make([]int, 0)
			if PERCEPTRON {
				predictions = AI.GetPrediction(imageFragmentTo28x28PerceptronVersion(blockRect, img), perceptrons)
			} else {
				predictions = NN.GetDigitPredictionFromImageArray([]int{28, 28}, imageFragmentTo28x28cnnVersion(blockRect, img))
				//predictions = NN.GetDigitPredictionFromImageArray([]int{28, 28}, imageFragmentTo28x28cnnVersion(image.Rect(start+int(2*float64(borders)), minY+borders, finish-1, maxY-borders), img))
			}

			// matching predictions and digits
...
// 	//init, _ := getImageFromFile("pic8.png") // TODO 5, 8, 9
// 	//img := imageToGrayScale(init)
// 	//perceptrons := AI.InitializePerceptronMesh()
// 	//arr := make([]int, 28*28)
// 	//for i := 0; i < 28; i++ {
// 	//	for j := 0; j < 28; j++ {
// 	//		arr[i*28+j] = int(img.GrayAt(i, j).Y)
// 	//	}
// 	//}
// 	//fmt.Println(AI.GetPrediction(arr, perceptrons))
